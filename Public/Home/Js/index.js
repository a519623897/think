...
$(function () {
	...
	$( '#phiz img' ).click( function () {
		...
		var obj = $( 'textarea[name=common]' );
		obj.val(obj.val() + phiz);
	} );
    $('#send-btn').click(function () {
        var title = $('input[name=title]');
        var common = $('textarea[name=common]');
        if(title.val()==''){
            alert('标题不能为空');
            title.focus();
            return;
        ...
        $.post(ajaxSubmitUrl,{title:title.val(),common:common.val()},function(data){
            if(data.status){
             var str = '<dl class="paper a1">';
                str += '<dt><span class="username">'+data.title+'</span>';
                str += '<span class="num">No.'+ data.id +'</span>';
                str += '</dt><dd class="content">'+ data.common +'</dd>';
